  ...
  const blingFetchContacts = async (page = 1, salesMap?: Map<string, any>): Promise<Partial<Client>[]> => {
    ...
    return contatos.map((c: any): Partial<Client> => {
      ...
      return {
        ...
        potencialMapeado: totalPeriodo > 0,
        tier: 'C', // Será recalculado no confirm
        score: 0,
        ultimaInteracao: new Date().toISOString().split('T')[0],
        notas: `Importado do Bling. Histórico 24m: ${sales.count} pedidos, total faturado ${fmt(totalPeriodo)}.`,
        riscoOp: 'Baixa',
        relacEstrategico: totalPeriodo > 100000 ? 'Alta' : (totalPeriodo > 30000 ? 'Média' : 'Baixa'),
        nurtureStep: 0,
        pipelineStage: 0,
